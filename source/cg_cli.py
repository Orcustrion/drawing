    ...
    with open(input_file, 'r') as fp:
        ...
        while line:
            ...
            if line[0] == 'resetCanvas':
                ...
                _,tran_list,_,_=item_dict[line[1]]
                tran_result=alg.translate(tran_list,int(line[2]),int(line[3]))
                item_dict[line[1]][1]=tran_result
            elif line[0] == 'rotate':
                _,tran_list,_,_=item_dict[line[1]]
                tran_result=alg.rotate(tran_list,int(line[2]),int(line[3]),int(line[4]))
                item_dict[line[1]][1]=tran_result
            elif line[0] == 'scale':
                _,tran_list,_,_=item_dict[line[1]]
                tran_result=alg.scale(tran_list,int(line[2]),int(line[3]),float(line[4]))
                item_dict[line[1]][1]=tran_result
            elif line[0] == 'clip':
                _,tran_list,_,_=item_dict[line[1]]
                tran_result=alg.clip(tran_list,int(line[2]),int(line[3]),int(line[4]),int(line[5]),str(line[6]))
                item_dict[line[1]][1]=tran_result
